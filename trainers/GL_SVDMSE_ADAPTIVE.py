...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, idx=None):
        ...
        # Compute text features for global and projected local
        ...
        if self.training:
            # --- Original training forward (unchanged) ---
        ...
        # All three logits without logit_scale (unscaled similarity)
        local_logits = image_features @ text_features.t()                     # [B, n_cls]
        global_logits = image_features @ text_features_global.t()             # [B, n_cls]
        projected_logits = image_features @ text_features_projected_local.t() # [B, n_cls]

        if self.infer_mode == "local":
            # Original FedPHA local-only inference
            return logit_scale * local_logits

        # --- Adaptive inference (using scaled logits for entropy/residual) ---
        # Scale logits so softmax produces reasonable probability distributions
        ...
        # Compute probability distribution via softmax
        prob = F.softmax(local_logits_f, dim=-1)  # [B, n_cls]

        # Prediction entropy (raw)
        entropy = -(prob * torch.log(prob + eps)).sum(dim=-1)  # [B]

        # Normalized entropy: divide by log(num_classes), NOT num_classes
        ...
        # Projection residual: ||local - projected||_2 / (||local||_2 + eps)
        ...
        norm_residual = (residual - res_min) / (res_max - res_min + eps)  # [B]
        norm_residual = torch.clamp(norm_residual, 0.0, 1.0)

        # [Experiment] Sigmoid normalization (batch-independent)
        # norm_residual = torch.sigmoid(residual - 1.0)  # [B]

        # Sample-level fusion weight
        alpha = torch.sqrt(norm_entropy * norm_residual + eps)
        alpha = torch.clamp(alpha, 0.0, 1.0)
        alpha = alpha.detach().unsqueeze(1).to(local_logits_scaled.dtype)  # [B, 1]

        # Fused logits (already scaled, do NOT multiply by logit_scale again)
        final_logits = (1.0 - alpha) * local_logits_scaled + alpha * global_logits_scaled  # [B, n_cls]

        # One-time log
        if not hasattr(self, "_adaptive_printed"):
            print(
                f"[Adaptive Inference] alpha mean={alpha.mean().item():.4f}, "
                ...
                f"raw_entropy mean={entropy.mean().item():.4f}, "
                f"raw_residual mean={residual.mean().item():.4f}"
            )
...
class GL_SVDMSE_ADAPTIVE(TrainerX):
    """
    FedPHA + Adaptive Inference.
    Based on GL_SVDMSE (FedPHA).
    
    Training is identical to GL_SVDMSE.
    Inference supports two modes:
      - "local":    original FedPHA local-only inference.
      - "adaptive": dynamic fusion of local and global logits based on
                    prediction entropy and projection residual.
    """
